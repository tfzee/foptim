...
Goals:
        ...
        Also compare the performance of qsort(), quicksort template, and
std::sort
...
Assumptions:
        ...
        6) std::sort is faster than a naive quicksort template using the same
functor


Since qsort's comparison function must return int (less than 0, 0, greater than
0) and std::sort's must return a bool, it is not possible to test them with each
        other's comparator.
...
void quicksort_function(double *begin, double *end,
