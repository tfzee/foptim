...
public:
  ...
  utils::FStableVec<std::unique_ptr<GlobalData>> storage_global;
  ...
  Instr insert_instr(InstrData v) {
    return Instr(this->storage_instr.push_back(std::move(v)));
  }

  BasicBlock insert_bb(BasicBlockData v) {
    return BasicBlock(this->basic_blocks.push_back(std::move(v)));
  }

  BBArgument insert_bb_arg(BBArgumentData v) {
    return BBArgument(this->bb_args.push_back(std::move(v)));
  }
  ...
  ConstantValueR insert_constant(ConstantValue v) {
    return ConstantValueR(this->storage_constant.push_back(std::move(v)));
  }
  ...
  TypeR insert_type(AnyType v) {
    return TypeR(this->storage_type.push_back(std::move(v)));
  }
};
