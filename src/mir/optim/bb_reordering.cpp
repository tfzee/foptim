...
void BBReordering::apply(MFunc &func, const conf::CompConf &) {
  ...
  auto insert_after = [&new_bb_vals, &func](u32 start_i, u32 target_bb) {
    func.bbs.insert(func.bbs.begin() + start_i + 1,
    ...
    for (u32 i = start_i + 1; i < func.bbs.size(); i++) {
      new_bb_vals[i] = new_bb_vals[i] + 1;
    }
    new_bb_vals[target_bb] = start_i + 1;

    if (start_i + 1 <= target_bb) {
  ...
  //   if (t1.is(GBaseSubtype::unreach)) {
  //     insert_after(func.bbs.size() - 1, bb1_i);
  //     continue;
  //   }
  // }
  ...
  for (size_t bb1_i = 0; bb1_i < func.bbs.size(); bb1_i++) {
    ...
    auto &t1 = func.bbs[bb1_i].instrs.back();
    if (t1.is(GBaseSubtype::ret) || t1.is(GBaseSubtype::unreach)) {
      continue;
    }
