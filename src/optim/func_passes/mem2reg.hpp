...
using AllocToPhiLoc = FMap<fir::Instr, std::unordered_set<u32>>;
...
static void phi_insert_locations(fir::Function &func, fir::Instr alloca_instr,
  ...
  // consider
  FVec<u32> blocks_containing_store{};
  ...
  FVec<u32> blocks_to_consider = blocks_containing_store;
...
static inline void decide_value_store(fir::Instr instr, size_t &i,
  ...
  current_variable_value.back().insert(
      {instr->args[0], {block, instr->args[1]}});
