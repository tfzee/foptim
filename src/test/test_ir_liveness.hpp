...
TEST(LivenessTest, SingleBB) {
  ...
  EXPECT_EQ(lives.live_variables[ValueR(entry_bb, 0)].size(), 1);
...
TEST(LivenessTest, AcrossBB) {
  ...
  end_bb.add_arg(ctx.data->get_int_type(32));

  builder.at_end(entry_bb);
  auto branch = builder.build_branch(end_bb);
  branch.add_bb_arg(0, ValueR{entry_bb, 0});
  builder.at_end(end_bb);
  auto x = builder.build_int_add(ValueR{entry_bb, 0}, ValueR{end_bb, 0});
  auto y = builder.build_int_add(ValueR{entry_bb, 1}, x);
  ...
  EXPECT_EQ(lives.live_variables[ValueR(entry_bb, 0)].size(), 2);
  EXPECT_EQ(lives.live_variables[ValueR(entry_bb, 1)].size(), 2);
  EXPECT_EQ(lives.live_variables[ValueR(end_bb, 0)].size(), 1);
  EXPECT_EQ(lives.live_variables[x].size(), 1);
  ...
  auto arg_live1 = lives.live_variables[ValueR(entry_bb, 0)][0];
  auto arg_live2 = lives.live_variables[ValueR(entry_bb, 0)][1];
  ...
  auto arg2_live1 = lives.live_variables[ValueR(entry_bb, 1)][0];
  auto arg2_live2 = lives.live_variables[ValueR(entry_bb, 1)][1];
  ...
  auto bb_arg_live = lives.live_variables[ValueR(end_bb, 0)][0];
