#include "backend3_instr_gen.hpp"
...
void emit_operand(const fmir::MArgument &arg, ZydisEncoderOperand &operand,
  ...
  case fmir::MArgument::ArgumentType::Imm:
    ...
    case fmir::Type::Float32: {
      f32 v =
          std::bit_cast<f32>(static_cast<u32>(std::bit_cast<u64>(arg.immf)));
      int i;
      memcpy(&i, &v, sizeof i);
      operand.imm.s = i;
      break;
  ...
  case fmir::MArgument::ArgumentType::MemVReg:
    ...
    operand.mem.size = get_size(arg.ty);
    return;
  case fmir::MArgument::ArgumentType::StackSlot:
    TODO("these should be gone hmmmm");
...
size_t emit_gjmp(ZydisEncoderRequest &req, const fmir::MInstr &instr,
  ...
  (void)proepiloguetype;
  size_t length = 9999;
  switch (static_cast<fmir::GJumpSubtype>(instr.sop)) {
  ...
  case fmir::GJumpSubtype::fcmp_une: {
    ...
    emit_operand(instr.args[1], req.operands[0], reloc_map, out_buff, 0);
    emit_operand(instr.args[2], req.operands[1], reloc_map, out_buff, 1);
    req.operand_count = 2;
    u64 off = 0;
    off = emit(out_buff, off, &req);
    req.mnemonic = mem;
  ...
  case fmir::GJumpSubtype::icmp_mul_overflow: {
    ...
    u64 off = 0;
    if (b.type == ZYDIS_OPERAND_TYPE_IMMEDIATE) {
...
size_t emit_gconv(ZydisEncoderRequest &req, const fmir::MInstr &instr,
  ...
  (void)proepiloguetype;
  size_t length = 9999;
  switch (static_cast<fmir::GConvSubtype>(instr.sop)) {
  ...
  case fmir::GConvSubtype::UI2FL: {
    ...
    if (instr.args[0].ty == fmir::Type::Float64 &&
      ...
      ZydisEncoderEncodeInstruction(&req_vcvt, tmp, &(len_vcvt = sizeof(tmp)));
      ZydisEncoderEncodeInstruction(&req_vadd, tmp, &(len_vadd = sizeof(tmp)));
      ...
      req.operand_count = 1;
      req.operands[0].type = ZYDIS_OPERAND_TYPE_IMMEDIATE;
      req.operands[0].imm.s = len_shr + len_adc + len_vcvt + len_vadd + len_jmp;
      off = emit(out_buff, off, &req);
      // SHR inp, 1
...
size_t emit_garith(ZydisEncoderRequest &req, const fmir::MInstr &instr,
  ...
  (void)reloc_map;
  (void)proepiloguetype;
  switch (static_cast<fmir::GArithSubtype>(instr.sop)) {
  ...
  case fmir::GArithSubtype::mul2: {
    ...
    i32 val_casted = static_cast<i32>(static_cast<i64>(instr.args[1].imm));
  ...
  case fmir::GArithSubtype::smul3: {
    ...
    req.mnemonic = ZYDIS_MNEMONIC_IMUL;
    // TODO: need to check if its fits correclty
    i64 val_casted = static_cast<i64>(instr.args[1].imm);
    if (req.operands[1].type == ZYDIS_OPERAND_TYPE_IMMEDIATE &&
        (val_casted <= std::numeric_limits<i32>::min() ||
         val_casted >= std::numeric_limits<i32>::max())) {
      TODO("cant do this big of a constant");
...
size_t emit_gcmov(ZydisEncoderRequest &req, const fmir::MInstr &instr,
  ...
  case fmir::GCMovSubtype::cmov: {
    ...
    if (instr.args[0].ty == fmir::Type::Float32 ||
      ...
      req.operand_count = 1;
      req.operands[0].type = ZYDIS_OPERAND_TYPE_IMMEDIATE;
      req.operands[0].imm.s = static_cast<i64>(last_instr_size);
...
size_t emit_gvec(ZydisEncoderRequest &req, const fmir::MInstr &instr,
  ...
  (void)proepiloguetype;
  size_t length = 9999;
  switch (static_cast<fmir::GVecSubtype>(instr.sop)) {
...
size_t emit_x86(ZydisEncoderRequest &req, const fmir::MInstr &instr,
  ...
  (void)proepiloguetype;
  size_t length = 9999;
  switch (static_cast<fmir::X86Subtype>(instr.sop)) {
  ...
  case fmir::X86Subtype::LockXAdd2: {
    for (auto i = 0; i < req.operand_count; i++) {
      emit_operand(instr.args[i], req.operands[i], reloc_map, out_buff, i);
    }
    req.mnemonic = ZYDIS_MNEMONIC_XADD;
    req.prefixes |= ZYDIS_ATTRIB_HAS_LOCK;
    ZY_ASS_REQ(ZydisEncoderEncodeInstruction(&req, out_buff, &length), req);
    return length;
  }
  ...
  case fmir::X86Subtype::vextractf64x4: {
    for (auto i = 0; i < req.operand_count; i++) {
      emit_operand(instr.args[i], req.operands[i], reloc_map, out_buff, i);
    }
    req.mnemonic = ZYDIS_MNEMONIC_VEXTRACTF64X4;
    ZY_ASS_REQ(ZydisEncoderEncodeInstruction(&req, out_buff, &length), req);
    return length;
  }
  case fmir::X86Subtype::vextractf64x2: {
    for (auto i = 0; i < req.operand_count; i++) {
      emit_operand(instr.args[i], req.operands[i], reloc_map, out_buff, i);
    }
    req.mnemonic = ZYDIS_MNEMONIC_VEXTRACTF64X2;
    ZY_ASS_REQ(ZydisEncoderEncodeInstruction(&req, out_buff, &length), req);
    return length;
  }
  ...
  case fmir::X86Subtype::vinsertps: {
    for (auto i = 0; i < req.operand_count; i++) {
      emit_operand(instr.args[i], req.operands[i], reloc_map, out_buff, i);
    }
    req.mnemonic = ZYDIS_MNEMONIC_VINSERTPS;
    ZY_ASS_REQ(ZydisEncoderEncodeInstruction(&req, out_buff, &length), req);
    return length;
  }
  ...
  case fmir::X86Subtype::vpermil: {
    ...
    bool is_f32 = instr.args[1].ty == fmir::Type::Float32;
    req.mnemonic = is_f32 ? ZYDIS_MNEMONIC_VPERMILPS : ZYDIS_MNEMONIC_VPERMILPD;
    ZY_ASS_REQ(ZydisEncoderEncodeInstruction(&req, out_buff, &length), req);
    return length;
  }
  ...
  case fmir::X86Subtype::vpextr: {
    ...
    assert(req.operand_count == 3);
    if (instr.args[0].is_fp() && instr.args[0].ty == fmir::Type::Float32 &&
        instr.args[1].is_vec_reg()) {
      req.mnemonic = ZYDIS_MNEMONIC_VINSERTPS;
      req.operand_count = 4;
      req.operands[3] = req.operands[2];
      req.operands[2] = req.operands[1];
      req.operands[1] = req.operands[0];
