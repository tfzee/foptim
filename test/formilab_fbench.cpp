// RUN: clang++ -O0 -fno-exceptions %s -o %t.ll -S -emit-llvm
// RUN: %foffcc %t.ll %t.o
// RUN: clang++ -static-libstdc++ %t.o -o %t.out
// RUN: result=$(bash -c '(%t.out); echo Result:$?' 2>&1)
// RUN: echo $result | FileCheck %s

// CHECK: No errors in results. Result:0
...
public:
#if !FLOAT_MPFR
  constexpr static Wavelength A = 7621.0, B = 6869.955, C = 6562.816,
                              D = 5895.944, E = 5269.557, F = 4861.344,
                              Gprime = 4340.477, H = 3968.494;
#else
...
#if FLOAT_MPFR
Wavelength SpectralLine::A = 7621.0, SpectralLine::B = 6869.955,
           SpectralLine::C = 6562.816, SpectralLine::D = 5895.944,
           SpectralLine::E = 5269.557, SpectralLine::F = 4861.344,
           SpectralLine::Gprime = 4340.477, SpectralLine::H = 3968.494;
#endif
...
class Surface {
public:
  Real curvature_Radius, index_Of_Refraction, dispersion, edge_Thickness;
  ...
  void show(ostream &os) {
    os << setw(8) << curvature_Radius << setw(8) << index_Of_Refraction
       << setw(8) << dispersion << setw(8) << edge_Thickness << endl;
  }
};
...
public:
  ...
  Design(void) { nSurfaces = 0; }

  Design(Real ca, unsigned int ns) {
    clearAperture = ca;
    if (ns >= MaxSurfaces) {
      printf("number of surfaces exceeds MaxSurfaces");
      abort();
    }
    nSurfaces = ns;
  }
  ...
  void setSurf(int n, Surface *s) {
    if (n >= MaxSurfaces) {
      printf("surface index exceeds MaxSurfaces");
      abort();
    }
    surf[n] = s;
  }
...
private:
  ...
  Wavelength line;
  unsigned int cSurf;
  Real radius_of_curvature, object_distance, ray_height, axis_slope_angle,
      from_index, to_index;
...
public:
  ...
  void set(Design &des, Wavelength w, AxialIncidence ai) {
    ...
    line = w;
    cSurf = 0;
    radius_of_curvature = object_distance = axis_slope_angle = to_index = 0;
  ...
  void show(ostream &os) {
    ...
    os << "OD: " << object_distance << "  SA: " << axis_slope_angle
       << "  ROC: " << radius_of_curvature << "  RH: " << ray_height
       << "  FI: " << from_index << "  TI: " << to_index << endl;
    os.precision(op);
  }
};
...
bool TraceContext::transitSurface(void) {
  ...
  if (to_index > 1) {
    to_index +=
        ((SpectralLine::D - line) / (SpectralLine::C - SpectralLine::F)) *
  ...
  if (axial_incidence == Paraxial_Ray) {
    ...
    if (radius_of_curvature != 0) {
      ...
      const Real rayheightprime = odz ? ray_height : object_distance * asaprime;
    ...
    if (radius_of_curvature != 0) {
      ...
      const Real rayheightprime = odz ? ray_height : object_distance * asaprime;
      const Real objectdistanceprime =
          ((radius_of_curvature * sin(asaprime + iang)) * cot(asadoubleprime)) +
          sagitta;
      ...
      const Real rang =
          -(asin((from_index / to_index))) * sin(axis_slope_angle);

      object_distance =
          object_distance *
          ((to_index * cos(-rang)) / (from_index * cos(axis_slope_angle)));
...
void DesignEvaluation::evaluate(void) {
  ...
  offenseAgainstSineCondition =
      1 - (dParaxialOD * dParaxialSA) / (sin(dMarginalSA) * dMarginalOD);
  ...
  maxAxialChromaticAberration =
      maxLongitudinalSphericalAberration; // Same criterion
}
...
void DesignEvaluation::report(void) {
  ...
  const static char mp[] = "    (Maximum permissible):              %s",
                    ry[] = "%15s   %s  %s", qf[] = "%16.11Qf";
  char b1[80], b2[80];
...
#undef Qe
  ...
  const static char mp[] =
      "    (Maximum permissible):              %16.11" RealFormat,
                    ry[] = "%15s   %21.11" RealFormat "  %14.11" RealFormat;
